...
DEFAULT_KB = {
    ...
    "who are you?":"I am AI to answer simple questions",
    ...
    "what is your birthdate?":"As virtual assistant, i don't have any birthdate but i made on date 29 september 2025",
...
def _safe_eval(expr):
    ...
    try: return str(eval(expr, {"__builtins__":None}, SAFE_MATH))
    except Exception as e: return "Math error: "+str(e)

# ---------- AI ----------
def normalize(txt): return re.sub(r'[^a-z0-9\s]', '', txt.lower().strip())
...
def respond(text):
    ...
    if text.lower().startswith("teach:"):
        ...
        except: return "Teach format: teach:question=>answer"
    ...
    if text.lower().startswith("calc "): return _safe_eval(text[5:])
    elif math_expr: return _safe_eval(text)
    if n in knowledge: return knowledge[n]
    if any(w in n for w in ["hello","hi","hey"]): return f"Hello {user.get('name','')}! Ask 'help'."
    if "time" in n: return datetime.now().strftime("%H:%M:%S")
    if "date" in n: return datetime.now().strftime("%A, %d %B %Y")
...
class Bubble(Label):
    def __init__(self, text, who, **kwargs):
        ...
        with self.canvas.before:
            if who=="ai": Color(0.12, 0.56, 1, 0.85)  # Blue color for AI
            else: Color(0.2, 0.8, 0.3, 0.85)         # Green color for user
...
class ChatUI(FloatLayout):
    def __init__(self, **kwargs):
        ...
        top.add_widget(Label(text="[b]JD AI[/b]",markup=True,size_hint_x=0.5))
        btn_wall = Button(text="Wallpaper",size_hint_x=0.25,background_normal='',background_color=(0.4,0.6,1,1))
        btn_wall.bind(on_release=self.open_wallpapers)
        top.add_widget(btn_wall)
        btn_save = Button(text="Save",size_hint_x=0.25,background_normal='',background_color=(0.8,0.3,0.6,1))
